...
class TranscriptionService:
    ...
    def process_and_store(self,
                         ...
                         language: str = None,
                         run_analysis: bool = False) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            run_analysis: Whether to run initial analysis on the transcript
        ...
        try:
            ...
            storage_path = f"{transcript_id}{os.path.splitext(original_filename)[1]}"
            ...
            if run_analysis and not self.demo_mode:
                try:
                    # Import here to avoid circular imports
                    from analysis_svc.pipeline import run_analysis_pipeline
                    import asyncio
                    
                    # Run analysis in a blocking way (this is a sync method)
                    logger.info(f"Running initial analysis for transcript {transcript_id}")
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    analysis_result = loop.run_until_complete(run_analysis_pipeline(transcript_id))
                    loop.close()
                    
                    if analysis_result and "sales_data" in analysis_result:
                        logger.info(f"Initial analysis complete for transcript {transcript_id}")
                        result["sales_analysis"] = analysis_result["sales_data"]
                    else:
                        logger.warning(f"Initial analysis did not return expected data for transcript {transcript_id}")
                        
                except Exception as analysis_error:
                    logger.error(f"Error running initial analysis: {str(analysis_error)}")
